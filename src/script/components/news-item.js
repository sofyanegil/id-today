...
class NewsItem extends HTMLElement {
  set news(news) {
    this._news = news;
    this.setAttribute('class', 'card sm:basis-full md:basis-1/4 lg:basis-1/5 rounded-xl overflow-hidden shadow-md shadow-slate-600 m-3 border-1 border-slate-400');
    this.render();
  }
  ...
  render() {
    this.innerHTML = `
        ...
        <img class="lazyload w-full max-h-30" src="images/placeholder.png" data-src="${this._news.thumbnail}" alt="${this._news.title}" />
        <div class="p-2 bg-slate-100">
          <h1 class="text-xl font-bold">${this._news.title}</h1>
          <p class="text-sm text-justify text-ellipsis overflow-hidden">${this._news.description}...</p>
        </div>
